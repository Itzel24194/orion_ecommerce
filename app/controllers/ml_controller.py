...
def admin_entrenar_modelo_logistico():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    resultado = MLService.entrenar_modelo_logistico(force_retrain=True, solo_clientes=True)
    return jsonify(resultado)

def admin_predecir_logistico():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    usuario_id = request.args.get('usuario_id')
    resultado = MLService.predecir_logistico(usuario_id, solo_clientes=True)
    return jsonify(resultado)

def admin_metricas_logistico():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    metricas = MLService.obtener_metricas_logistico()
    if not metricas:
        return jsonify({'success': False, 'message': 'Modelo logístico no entrenado'}), 404
    metricas['_id'] = str(metricas.get('_id'))
    return jsonify({'success': True, 'metricas': metricas})

def admin_matriz_confusion_logistico():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    cm = MLService.obtener_matriz_confusion_logistico()
    if cm is None:
        return jsonify({'success': False, 'message': 'Modelo logístico no encontrado'}), 404
    return jsonify({'success': True, 'confusion_matrix': cm, 'labels': ['No Abandona', 'Abandona']})

def admin_reporte_logistico():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    reporte = MLService.obtener_reporte_logistico()
    if reporte is None:
        return jsonify({'success': False, 'message': 'Modelo logístico no encontrado'}), 404
    return jsonify({'success': True, 'reporte': reporte})

def admin_importancia_logistico():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    importancia = MLService.obtener_importancia_logistico()
...
# ================================================================
# 5. MÉTRICAS Y MATRIZ (RANDOM FOREST)
# ================================================================
...
def admin_reporte_clasificacion():
    ...
    try:
        ...
        return jsonify({'success': True, 'reporte': report_serializable})
    except Exception as e:
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500
...
# ================================================================
# 7. UMBRALES (solo para configuración, no se muestra en UI)
# ================================================================
...
def admin_dashboard_ml():
    ...
    respuesta = {
        ...
        'modelos': {
            ...
            'logistico': {'existe': False, 'metricas': {}}
        },
        'umbrales': {
            'activo': MLService.UMBRAL_ABANDONO_BAJO,
            'bajo': MLService.UMBRAL_ABANDONO_MEDIO,
    ...
    logistico_data = MLService._cargar_modelo(MLService.MODELO_LOGISTICO)
    if logistico_data:
        respuesta['modelos']['logistico']['existe'] = True
        respuesta['modelos']['logistico']['metricas'] = {
            'accuracy': logistico_data.get('accuracy'),
            'n_muestras': logistico_data.get('n_muestras'),
            'fecha_entrenamiento': logistico_data.get('fecha_entrenamiento')
        }
    return jsonify(respuesta)


# ================================================================
# 9. IMPORTANCIA (RANDOM FOREST)
# ================================================================
...
def admin_obtener_datos_segmentacion():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    datos = MLService.obtener_datos_segmentacion()
    if not datos:
        return jsonify({'success': False, 'message': 'Datos de segmentación no disponibles'}), 404
    return jsonify({'success': True, **datos})


# ================================================================
# 11. CURVA ROC PARA REGRESIÓN LOGÍSTICA
# ================================================================

def admin_curva_roc_logistico():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    datos = MLService.obtener_curva_roc()
    if datos is None:
        return jsonify({'success': False, 'message': 'Modelo logístico no encontrado o datos insuficientes'}), 404
    return jsonify({'success': True, **datos})


# ================================================================
# 12. EVALUACIÓN DE K PARA SEGMENTACIÓN (CODO + SILUETA)
# ================================================================

def admin_segmentacion_codo_silueta():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
        return jsonify({'error': 'No autorizado'}), 403
    max_k = request.args.get('max_k', 10, type=int)
    max_k = max(2, min(max_k, 15))
    resultado = MLService.evaluar_codo_silueta(max_k)
...
# ================================================================
# 13. PCA PARA SEGMENTACIÓN
# ================================================================

def admin_segmentacion_pca():
    if 'user_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    usuario = Usuario.obtener_por_id(session['user_id'])
    if normalizar_rol(usuario.get('rol')) != 'admin':
    ...
    n_clusters = request.args.get('n_clusters', 4, type=int)
    n_clusters = max(2, min(n_clusters, 10))
    resultado = MLService.calcular_pca(n_clusters)
    if resultado is None:
        return jsonify({'success': False, 'message': 'Datos insuficientes'}), 400
    return jsonify({'success': True, **resultado})